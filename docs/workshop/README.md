...
| **NHS Product Owner** | Decomposes user journeys into user stories with NHS acceptance criteria, then groups them into journey-aligned batches and prioritises them into a build backlog so each batch can be demonstrated end-to-end. Run after Architect first pass, before Architect second pass. |
