---
applyTo: "**/tests/**,**/test_*,**/*Tests.cs,**/*Test.cs"
---
...
- **Unit tests**: MAY mock external dependencies (databases, APIs, file systems) using `Moq` or `NSubstitute` — this is standard isolation practice
- **Cloud services with no local emulator** (e.g. hosted AI/LLM APIs): unit tests MUST mock the SDK client using the test framework's mocking library (e.g. `Moq`/`NSubstitute` for .NET, `jest.mock` for JavaScript) — there is no alternative for local testing. An ADR authorising the integration is sufficient justification; a separate user story for the mock is not required. Integration tests should use the real service endpoint or be skipped with a clear reason if no sandbox exists
