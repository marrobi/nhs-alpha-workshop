...
Read `tech-stack.instructions.md` to determine the backend language, frontend framework, linter, test runner, coverage tool, dependency audit command, and IaC tool. Generate the workflow steps from the actual stack — do not hardcode language versions or tool names.
