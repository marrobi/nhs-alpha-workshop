...
- **Stop and ask when a required tool is missing** — if a required component (MCP server, Playwright browser, CLI, or external service) is unavailable or fails to execute, pause and ask the user how to proceed rather than degrading the result or fabricating output. See `.github/instructions/tool-availability.instructions.md`.
